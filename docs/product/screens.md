...
화면에 **존재하지 않는** 것: ~~로그인/회원,~~ 마이페이지, 지도, 후기/플레이리스트. 상세 페이지에 "관심/저장" 같은 로그인 의존 액션 없음.

> **v5 갱신(2026-06-13, V-2):** 로그인/회원이 **S3 로그인 화면**으로 도입됨(F9). 소셜 OAuth(카카오+구글). 인증은 선택적 — 비로그인도 모든 탐색 기능 사용 가능. 로그인 의존 액션(Following/기록)은 아직 범위 밖.

> **v5 갱신(2026-06-13):** 아티스트 필터(F8), 가격 밴드 칩(F6), 출연 아티스트 칩(F7) 추가. 기존 범위 밖이던 아티스트/가격대 UI가 v5 Phase 1에서 도입됨.

> **v0.2 갱신(2026-06-12, D10):** 공연명 검색바는 v0.1 제외였으나 **v0.2에서 도입**(F5). UI 배치는 §2.3a 참조. 그 외 항목은 여전히 범위 밖.
...
- **구성:**
  ...
  - 우: **v0.5(F9):** 비로그인 시 "로그인" text button → S3으로 이동. 로그인 시 프로필 아바타(24px circle, OAuth 프로필 이미지) + 이름(`label-large`) → 클릭 시 드롭다운(로그아웃 버튼). **v0.2(D10 갱신): 공연명 검색 입력 배치(§2.3a).**
...
필터바는 5개 필터(월/지역/장르/공연장/아티스트) + 초기화로 구성. **데스크톱은 한 줄 inline**, **모바일은 칩 요약 줄 + 바텀시트 편집**.
...
#### B-5a. 아티스트 필터 · F8 (자동완성)
- **목적:** 출연 아티스트 이름으로 목록을 좁힌다. MusicBrainz 매칭 기반 아티스트 엔티티를 필터 조건으로 사용.
- **UI:** "아티스트" 트리거 버튼(Filter chip, trailing &#9662;). 탭 시 드롭다운 자동완성 패널 표시.
  - 검색 입력(Text field, leading search 아이콘, placeholder "아티스트 검색"). **2자 이상** 입력 시 후보 목록 표시(debounce 300ms).
  - 후보 목록: 아티스트명 + (있으면) alias 표시. 항목 클릭 시 선택 확정.
- **선택 표시:** 선택된 아티스트는 필터바에 **Input chip**으로 표시(예: `아이유 x`). Trailing close(X) 버튼으로 제거.
- **다중 선택:** v0.1은 단일 선택. 선택 시 기존 선택 교체.
- **동작:** 선택 변경 시 debounce 300ms 후 재조회(B-6 공통 동작). URL 동기화: `?artist=<artistId>`.
- **빈 선택:** 아티스트 필터 미적용(전체).
...
### 2.3a 영역 B' — 공연명 검색 (v0.2, D10 갱신) · F5

> **연결 기능:** F5(공연명 검색). v0.2 신규 영역. 기존 필터바(B)와 **AND 결합**(F5.2).

- **목적:** 공연명(`shprfnm`)으로 목록을 좁힌다. 기존 기간/지역/장르/공연장/정렬 필터와 **동시 적용**(어느 것도 무시 안 함).
- **컴포넌트:** Search/Text field (M3, Type=Outlined, Leading=search 아이콘, Trailing=clear(X) — 입력값 있을 때만). §1.4 "검색 입력" 스펙 재사용(높이 56). placeholder "공연명 검색".
- **배치:**
  - 데스크톱: 필터바(B) **상단** 또는 상단바(A) 우측. 콘텐츠 최대폭 1200 기준 좌측 정렬 또는 우측 정렬 단일 입력(폭 280~360). 권장: 필터바 위 별도 줄(높이 56) 또는 상단바 우측. 구현 시 필터바 sticky와 함께 따라붙도록.
  - 모바일: 칩 요약 줄(B) **위** 풀폭 입력(좌우 패딩 16). [필터] 버튼과는 별개 줄.
- **동작(F5.3):**
  - 입력 시 **debounce 300ms** 후 자동 재조회(D6 일관, 별도 제출 버튼 없음). Enter 시 즉시 재조회(debounce 스킵).
  - 재조회 중 그리드는 §2.3 B-6과 동일(상단 progress 또는 스켈레톤 교체).
  - clear(X) 탭 → 검색어 비움 → 기존 필터만으로 재조회.
- **URL 동기화:** `?q=` 쿼리스트링 반영(F5.3, 공유/북마크). 뒤로가기 복원(D8) 포함.
- **가드(F5.4):** trim 후 2자 미만이면 `shprfnm` 미전송(전체 목록). 빈 입력=검색 비활성.
- **상태:**
  - 결과 0건 → 기존 **S1-E**(빈 상태) 재사용. 검색 전용 문구 별도 없음(v0.2 단순화).
  - 검색어가 있는 상태에서 "필터 초기화"(B-5) → 검색어도 함께 비움.
- **접근성:** 입력에 label("공연명 검색"), clear 버튼 aria-label. 키보드 포커스/Esc로 clear 가능.
...
| 아티스트 | 진입 칩 + Search + 자동완성 Menu + Input chip | 단일 | Input chip | (BFF artistId) | 2자 이상 입력 시 후보 표시(F8) |
...
- **출연 아티스트 칩(F7):** `prfcast`에서 추출 + MusicBrainz 매칭된 아티스트가 있으면, 텍스트 아래에 **Input chip** 목록으로 표시. 각 칩: 아티스트명(`label-large`), 클릭 시 해당 아티스트로 S1 목록 필터 이동(`/?artist=<artistId>`). 매칭 아티스트가 0명이면 칩 영역 미표시(텍스트만).
...
- **가격 밴드 칩(F6):** 가격 텍스트 아래(또는 옆)에 **PriceBandChip**(M3 tonal chip) 1개 표시. `parsePriceBand(pcseguidance)` 결과의 `label`을 칩 라벨로 사용. 밴드별 라벨: `무료/초대` / `~3만` / `3~7만` / `7만+` / `가변/미정`. 칩 스타일: tonal(M3 `secondary-container` 배경), `label-large`, 클릭 불가(정보 표시 전용). 밴드가 `VARIABLE`이면 칩 표시 생략 가능(선택).
...
| 공연명 검색 | S1 | `shprfnm` | v0.2(F5). 2자 미만 미전송, 기존 필터와 AND |
...
### 9a.1 레이아웃
- 중앙 정렬 단일 카드(max-width 400, 데스크톱/모바일 동일). 배경 `surface`. 카드 `surface-container-low`, 라운드 16, elevation level1.
- 카드 내부 패딩 32, 세로 정렬 gap 24.

### 9a.2 구성 (위→아래)
1. **로고** "Riff" (`headline-small`, `primary`). 중앙 정렬.
2. **제목** "로그인" (`title-large`, `on-surface`).
3. **소셜 버튼 2개** (세로 배열, gap 12):
   - **카카오로 시작하기** — Button(filled), 배경 카카오 브랜드 노랑(#FEE500), 텍스트 `#191919`, leading 카카오 아이콘. 높이 48, 풀폭.
   - **구글로 시작하기** — Button(outlined), `on-surface` 텍스트, leading 구글 로고. 높이 48, 풀폭.
4. **약관 동의 안내** (`body-small`, `on-surface-variant`, 중앙 정렬):
   - "로그인 시 [이용약관] 및 [개인정보처리방침]에 동의합니다."
   - 링크는 텍스트 내 밑줄(`primary`). v5 MVP에서는 간단한 정적 페이지로 연결.

### 9a.3 동작
- 카카오/구글 버튼 클릭 → Auth.js signIn 호출 → OAuth provider 리다이렉트.
- 인증 완료 → callbackUrl(기본 `/`)로 redirect.
- 이미 로그인 상태에서 `/login` 접근 → `/`로 redirect.

### 9a.4 접근성
- 버튼 aria-label: "카카오 계정으로 로그인", "구글 계정으로 로그인".
- 키보드 Tab 이동, Enter 실행.

---

## S4. Following 목록 페이지 (`/following`) · v6

### S4.1 개요
로그인 사용자의 팔로잉 아티스트 목록을 표시하는 전용 페이지.

- **URL**: `/following`
- **접근 제한**: 로그인 필수. 비로그인 접근 → `/login` redirect (middleware)
- **레이아웃**: S1 목록 페이지와 동일한 최대 폭(1280px), AppBar 포함.

### S4.2 구성 (위→아래)
1. **AppBar** — S1과 동일 헤더. "팔로잉 목록" 타이틀 표시(title-medium, on-surface).
2. **아티스트 목록** — ArtistChip 컴포넌트 나열 (wrap layout, gap 8).
   - 각 칩: 아티스트 이름 + 팔로잉 버튼 (언팔로우 가능).
3. **빈 상태** — 팔로잉 아티스트 0명 시: "팔로잉한 아티스트가 없습니다" (body-large, on-surface-variant, 중앙).

### S4.3 ArtistChip 팔로우 버튼 상태 (S1·S4 공통)
| 상태 | 표시 | 색상 |
|---|---|---|
| 비로그인 | 버튼 미표시 | — |
| 팔로우 가능 (로그인) | "팔로우" 텍스트 버튼 | secondary |
| 팔로잉 중 (로그인) | "팔로잉" 텍스트 버튼 | primary (filled) |

- 팔로우 버튼 클릭 → optimistic toggle → API 호출 → 실패 시 롤백.
- 팔로잉 버튼 호버 시 "언팔로우" 레이블로 전환 (UX 힌트).

### S4.4 AppBar 드롭다운 연결
- 로그인 상태 헤더 드롭다운에 "팔로잉 목록" 메뉴 항목 추가 → `/following`으로 이동.

---

## S5. Following Feed 페이지 (`/following-feed`) · v7

### S5.1 개요
로그인 사용자의 팔로잉 아티스트 출연 공연을 모아보는 전용 피드 페이지.
...
### S5.2 구성 (위→아래)
1. **AppBar** — S1과 동일 헤더.
2. **페이지 제목** — "팔로잉 공연" (`headline-small`, `on-surface`).
3. **공연 카드 그리드** — 기존 PerformanceCard 컴포넌트 재사용. 2~4열 반응형.
4. **빈 상태 (2종)**:
   - 팔로잉 아티스트 없음: "아직 팔로잉한 아티스트가 없어요" + `/following` 이동 버튼.
   - 다가오는 공연 없음: "팔로잉 아티스트의 다가오는 공연이 없습니다" (`body-large`, `on-surface-variant`).

### S5.3 AppBar 드롭다운 연결
- 로그인 상태 헤더 드롭다운에 "팔로잉 공연" 메뉴 항목 추가 → `/following-feed` 이동.
- 기존 "팔로잉 목록" (`/following`) 항목 아래 위치.
