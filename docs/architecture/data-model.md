...
| 필드 | 타입 | 설명 |
|---|---|---|
| `user_id` | TEXT | users.id (Auth.js user ID) |
| `artist_id` | INTEGER | artists.id |
| `created_at` | TEXT | ISO 8601 UTC (팔로우 시각) |
| PK | (user_id, artist_id) | 복합 기본키 — 중복 팔로우 방지 |

### 9.3 쿼리 패턴

- **팔로우 여부 확인**: `SELECT 1 FROM follows WHERE user_id=? AND artist_id=?`
- **팔로우 목록**: `SELECT artist_id FROM follows WHERE user_id=? ORDER BY created_at DESC`
- **팔로우 추가**: `INSERT OR IGNORE INTO follows VALUES (?,?,?)` — 중복 멱등
- **언팔로우**: `DELETE FROM follows WHERE user_id=? AND artist_id=?` — 없으면 no-op

### 9.4 결정 매핑

| 결정 | 항목 |
|---|---|
| V-13 | 팔로우 UI 진입점 = ArtistChip 인라인 버튼 |
| V-14 | Following 목록 = `/following` 별도 페이지 |
